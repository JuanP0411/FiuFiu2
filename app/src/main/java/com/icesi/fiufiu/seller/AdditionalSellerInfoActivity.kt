...
import com.icesi.fiufiu.databinding.ActivityAdditionalSellerInfoBinding
...
class AdditionalSellerInfoActivity : AppCompatActivity() {
    ...
    private fun sendSeller(){
        ...
        if(verifyBlankAdditionalInfoFields(marketName,marketDescription)){
            var service = Service(currentUser.marketID,currentUser.id,marketName,marketDescription,marketShortDescription,"",currentUser.phone)

            Firebase.firestore.collection("users").document(currentUser.id).set(currentUser).addOnSuccessListener {
                Firebase.firestore.collection("markets").document(currentUser.marketID).set(service).addOnSuccessListener {
                    loadImgToMarket()
